...
No clinical data are currently distributed with this repository. A de-identified MDA multi-annotator release is planned through TCIA; see [MDA_DATASET.md](MDA_DATASET.md). Obtain data only through the relevant institutional or public archive process and comply with approvals, data-use agreements and de-identification requirements.
...
MDA data use `<root>/<id>/CT`, `T1dr`, `T2dr`, `GTVt.nii`, `GTVn.nii` and `GTVn_clicks.nii.gz`. The archived loader appends no extension to the image-channel names. PET was excluded from the MDA experiments. This local research layout is not a specification for the future TCIA package; an archive-to-code conversion step may be required after release.
