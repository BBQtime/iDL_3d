...
def symmetric_focal_loss(delta=0.7, gamma=2.0):
    ...
    # gamma : float, optional
    #     controls degree of down-weighting of easy examples, by default 2.0
    # """
    ...
    def loss_function(y_true, y_pred):
        ...
        back_ce = (
            torch.pow(1 - y_pred[:, 0, :, :, :], gamma) * cross_entropy[:, 0, :, :, :]
        )
        ...
        gtvt_ce = (
            torch.pow(1 - y_pred[:, 1, :, :, :], gamma) * cross_entropy[:, 1, :, :, :]
        )
        gtvt_ce = delta * gtvt_ce
        gtvn_ce = (
            torch.pow(1 - y_pred[:, 2, :, :, :], gamma) * cross_entropy[:, 2, :, :, :]
        )
        gtvn_ce = delta * gtvn_ce

        loss = torch.mean(
            torch.sum(torch.stack([back_ce, gtvt_ce, gtvn_ce], dim=-1), dim=-1)
        )
...
def symmetric_focal_tversky_loss(delta=0.7, gamma=0.75):
    ...
    def loss_function(y_true, y_pred):
        ...
        gtvt_dice = (1 - dice_class[:, 1]) * torch.pow(1 - dice_class[:, 1], -gamma)
        gtvn_dice = (1 - dice_class[:, 2]) * torch.pow(1 - dice_class[:, 2], -gamma)

        # Average class scores
        loss = torch.mean(torch.stack([back_dice, gtvt_dice, gtvn_dice], dim=-1))
        return loss
...
def asymmetric_focal_loss(delta=0.7, gamma=2.0):
    def loss_function(y_true, y_pred):
        ...
        # fore_ce = delta * fore_ce
        gtvt_ce = cross_entropy[:, 1, :, :, :]
        gtvt_ce = delta * gtvt_ce
        gtvn_ce = cross_entropy[:, 2, :, :, :]
        gtvn_ce = delta * gtvn_ce
