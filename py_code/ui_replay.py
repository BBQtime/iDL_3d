...
import cv2
import numpy as np
from custom import Debug, Dict, DirExplorer
...
class UiReplay(QMainWindow, Ui_Core):
    ...
    def _init_member_var(
        ...
        # DATASET_SPLIT_JSON_PATH[s.AU_1MM] and [s.AU_3MM] are the same
        dataset_split_au = Json.load(g.DATASET_SPLIT_JSON_PATH[s.AU_1MM])
        dataset_split_mda = Json.load(g.DATASET_SPLIT_JSON_PATH[s.MDA])
        self._patients = Dict()
        self._patients["au.test.inter"] = List(dataset_split_au[s.TEST_INTER])
        self._patients["au.test.exter"] = List(dataset_split_au[s.TEST_EXTER])
        self._patients["mda.test"] = List(dataset_split_mda[s.TEST])
        ...
        for i in [s.GTVT, s.GTVN]:
            self._idl_id[i] = s.BASELINE
            self._idl_round[i] = s.ROUND_00
        ...
        self.__gtvt_selected_slices_2d[s.HORIZONTAL] = []
        self.__gtvt_selected_slices_2d[s.VERTICAL] = []

        self.__total_slices_count_2d = Dict()
        self.__total_slices_count_2d[s.HORIZONTAL] = 0
        self.__total_slices_count_2d[s.VERTICAL] = 0

    def _clear_img_data(self):
        for i in [s.GTVT, s.GTVN]:
            self.__scores[i][s.DSC] = None
            self.__scores[i][s.MSD] = None
            self.__scores[i][s.HD95] = None
        ...
        # set image plane
        for i in [s.TRANSVERSE, s.CORONAL, s.SAGITTAL]:
    ...
    #     if self._img_plane == s.SAGITTAL:
    #         origin_width = self._3d_imgs[s.CT].shape[1]
    #         origin_height = self._3d_imgs[s.CT].shape[0]
    #         origin_height = round(
    #             origin_height * self._nii_spacing[2] / self._nii_spacing[1]
    #         )
    #     elif self._img_plane == s.CORONAL:
    #         origin_width = self._3d_imgs[s.CT].shape[2]
    #         origin_height = self._3d_imgs[s.CT].shape[0]
    #         origin_height = round(
    ...
    #     else:
    #         origin_width = self._3d_imgs[s.CT].shape[2]
    #         origin_height = self._3d_imgs[s.CT].shape[1]
    ...
    def __init_color(self):
        self._color = Dict()
        self._color[s.GTVT_LABEL] = (0, 255, 255)  # light blue
        self._color[s.GTVN_LABEL] = (0, 150, 255)  # dark blue
        self._color[s.GTVT_PRED] = (255, 255, 0)  # yellow
        self._color[s.GTVN_PRED] = (255, 128, 0)  # orange
        self._color[s.GTVT_ANNOTATION] = (0, 255, 64)  # green
        self._color[s.GTVN_CLICKS] = (255, 70, 200)  # pink
        self._color[s.SCORE_TEXT] = self._color[s.GTVT_ANNOTATION]

    def _init_ui_names(self):
        self._img_qlabel = Dict()
        self._img_qlabel[s.CT] = self._img_qlabel_ct
        self._img_qlabel[s.PT] = self._img_qlabel_pt
        self._img_qlabel[s.MR1] = self._img_qlabel_mr1
        self._img_qlabel[s.MR2] = self._img_qlabel_mr2

        self._text_label = Dict()
        self._text_label[s.BASELINE] = self._text_label_baseline
        self._text_label[s.PATIENT] = self._text_label_patient
        self._text_label[s.IDL_GTVT] = self._text_label_idl_gtvt
        self._text_label[s.IDL_GTVN] = self._text_label_idl_gtvn
        self._text_label[s.BRIGHT] = self._text_label_bright
        self._text_label[s.CONTRAST] = self._text_label_contrast
        self._text_label[s.ZOOM] = self._text_label_zoom
        ...
        self._combox = Dict()
        self._combox[s.BASELINE] = self._combox_baseline
        self._combox[s.PATIENT] = self._combox_patient
        self._combox[s.IDL_GTVT] = self._combox_idl_gtvt
        self._combox[s.IDL_GTVN] = self._combox_idl_gtvn
        ...
        self.__radio_btn[s.CT] = self._radio_btn_ct
        self.__radio_btn[s.PT] = self._radio_btn_pt
        self.__radio_btn[s.MR1] = self._radio_btn_mr1
        self.__radio_btn[s.MR2] = self._radio_btn_mr2
        self.__radio_btn[s.TRANSVERSE] = self._radio_btn_transverse
        self.__radio_btn[s.CORONAL] = self._radio_btn_coronal
        self.__radio_btn[s.SAGITTAL] = self._radio_btn_sagittal
        ...
        self.__slider[s.ZOOM] = self._slider_zoom
    ...
    def _init_side_bar(self):
        ...
        # set text
        self._text_label[s.BASELINE].setText("Choose Baseline")
        self._text_label[s.PATIENT].setText("Choose Patient")
        self._text_label[s.IDL_GTVT].setText("Choose iDL GTVt")
        self._text_label[s.IDL_GTVN].setText("Choose iDL GTVn")

        self._text_label[s.BRIGHT].setText("Brightness (CT)")
        self._text_label[s.CONTRAST].setText("Contrast (CT)")
        self._text_label[s.ZOOM].setText("Zoom In")

        self.__radio_btn[s.CT].setText("CT")
        self.__radio_btn[s.PT].setText("PT")
        self.__radio_btn[s.MR1].setText("MR-T1")
        self.__radio_btn[s.MR2].setText("MR-T2")
        self.__radio_btn[s.TRANSVERSE].setText("Transverse")
        self.__radio_btn[s.CORONAL].setText("Coronal")
        self.__radio_btn[s.SAGITTAL].setText("Sagittal")

        # set font
        self._font_bold = self._text_label[s.BASELINE].font()
        self._font_light = self._text_label[s.BASELINE].font()
        ...
        for i in [s.TRANSVERSE, s.CORONAL, s.SAGITTAL, s.CT, s.PT, s.MR1, s.MR2]:
            self.__radio_btn[i].setFont(self._font_bold)

        # set font of comboboxes
        for i in [s.BASELINE, s.PATIENT, s.IDL_GTVT, s.IDL_GTVN]:
        ...
        for i in [s.PATIENT, s.IDL_GTVT, s.IDL_GTVN]:
        ...
        self.__btn_group_bright_contrast.addButton(self.__radio_btn[s.CT])
        self.__btn_group_bright_contrast.addButton(self.__radio_btn[s.PT])
        self.__btn_group_bright_contrast.addButton(self.__radio_btn[s.MR1])
        self.__btn_group_bright_contrast.addButton(self.__radio_btn[s.MR2])
        self.__btn_group_plane = QButtonGroup()
        self.__btn_group_plane.addButton(self.__radio_btn[s.TRANSVERSE])
        self.__btn_group_plane.addButton(self.__radio_btn[s.CORONAL])
        self.__btn_group_plane.addButton(self.__radio_btn[s.SAGITTAL])

        # radio btns checked or not
        self.__radio_btn[s.CT].setChecked(True)
        self.__radio_btn[s.PT].setChecked(False)
        self.__radio_btn[s.MR1].setChecked(False)
        self.__radio_btn[s.MR2].setChecked(False)
        self.__radio_btn[s.TRANSVERSE].setChecked(True)
        self.__radio_btn[s.CORONAL].setChecked(False)
        self.__radio_btn[s.SAGITTAL].setChecked(False)
        ...
        for i in [s.PT, s.MR1, s.MR2]:
            self.__slider["bright.{}".format(i)].hide()
            self.__slider["contrast.{}".format(i)].hide()

        self.__slider[s.ZOOM].setMinimum(100)
        self.__slider[s.ZOOM].setMaximum(200)
        self.__slider[s.ZOOM].setValue(100)

        # connect ui to functions
        # (put the connections at last, because these functions will need the initialization above)
        self._combox[s.BASELINE].activated.connect(self._choose_baseline)
        ...
        self._combox[s.PATIENT].activated.connect(self._choose_patient)
        ...
        self._combox[s.IDL_GTVT].activated.connect(self._choose_idl_gtvt)
        ...
        self._combox[s.IDL_GTVN].activated.connect(self._choose_idl_gtvn)
        ...
        for i in [s.BRIGHT, s.CONTRAST]:
            for j in [s.CT, s.PT, s.MR1, s.MR2]:
    ...
    def _fill_combox_baseline(self):
        ...
        self._combox[s.BASELINE].addItems(baseline_id_list)

    def _refresh_side_bar(
        self, widgets_to_display: list = [s.BASELINE, s.PATIENT, s.IDL_GTVT, s.IDL_GTVN]
    ):
        left = 30
        top = 0
        text_height = 25
        ...
        radio_btn_width[s.CT] = radio_btn_width[s.PT] = 45
        radio_btn_width[s.MR1] = radio_btn_width[s.MR2] = 60
        radio_btn_width[s.TRANSVERSE] = 90
        radio_btn_width[s.CORONAL] = 70
        radio_btn_width[s.SAGITTAL] = 70
        radio_btn_gap = Dict()
        ...
        top += gap
        rect = QRect(left, top, width, text_height)
        self._text_label[s.ZOOM].setGeometry(rect)
        top += text_height
        rect = QRect(left, top, width, slider_height)
        self.__slider[s.ZOOM].setGeometry(rect)
        top += slider_height
    ...
    def _set_img_plane(self):
        ...
        if self._img_plane == s.TRANSVERSE:
            self.__gtvt_selected_slices_2d[
                s.HORIZONTAL
            ] = self.__get_gtvt_selected_slices_on_2d(s.CORONAL)
            self.__total_slices_count_2d[s.HORIZONTAL] = self._3d_imgs[s.CT].shape[1]
            self.__gtvt_selected_slices_2d[
                s.VERTICAL
            ] = self.__get_gtvt_selected_slices_on_2d(s.SAGITTAL)
            self.__total_slices_count_2d[s.VERTICAL] = self._3d_imgs[s.CT].shape[2]

        elif self._img_plane == s.CORONAL:
            self.__gtvt_selected_slices_2d[
                s.HORIZONTAL
            ] = self.__get_gtvt_selected_slices_on_2d(s.TRANSVERSE)
            self.__total_slices_count_2d[s.HORIZONTAL] = self._3d_imgs[s.CT].shape[0]
            self.__gtvt_selected_slices_2d[
                s.VERTICAL
            ] = self.__get_gtvt_selected_slices_on_2d(s.SAGITTAL)
            self.__total_slices_count_2d[s.VERTICAL] = self._3d_imgs[s.CT].shape[2]
            ...
            self.__total_slices_count_2d[s.VERTICAL] = self._3d_imgs[s.CT].shape[1]
    ...
    def __set_bright_contrast_modality(self):
        ...
        if self.__radio_btn[s.CT].isChecked():
            key_word = "CT"
        elif self.__radio_btn[s.PT].isChecked():
            key_word = "PT"
        elif self.__radio_btn[s.MR1].isChecked():
            key_word = "MR-T1"
        elif self.__radio_btn[s.MR2].isChecked():
            key_word = "MR-T2"
        else:
            Debug.error_exit("no radio button is checked")

        self._text_label[s.BRIGHT].setText("Brightness ({})".format(key_word))
        self._text_label[s.CONTRAST].setText("Contrast ({})".format(key_word))
    ...
    def _load_dataset_dir_and_nii_spacing(self):
        ...
        baseline_dir = os.path.join(g.TRAIN_RESULTS_DIR, self._baseline_id, s.BASELINE)
        ...
        if self._cur_patient in self._patients["au.test.inter"]:
            ...
            self._dataset_section = s.TEST_INTER

        elif self._cur_patient in self._patients["au.test.exter"]:
            self._dataset_ver = baseline_dataset_ver
            self._dataset_section = s.TEST_EXTER

        elif self._cur_patient in self._patients["mda.test"]:
            self._dataset_ver = s.MDA
            self._dataset_section = s.TEST
        else:
    ...
    def _fill_combox_patient(self):
        combox_patients = DirExplorer.get_sub_folders(
            os.path.join(g.TRAIN_RESULTS_DIR, self._baseline_id, s.BASELINE, s.PATIENTS)
        )
        ...
        self._combox[s.PATIENT].addItems(combox_patients)
        self._combox[s.PATIENT].setEnabled(True)
    ...
    def _choose_baseline(self):
        ...
        self._enable_arrow_btns(s.BASELINE)

        self._baseline_id = self._combox[s.BASELINE].currentText()
    ...
    def _choose_patient(self, idx: int = None, reset_patient: bool = True):
        ...
        # triggered by baseline combox update, and find cur patient in new baseline dir
            ...
            self._combox[s.PATIENT].setCurrentText(self._cur_patient)
        ...
        # reset comboboxes
        for i in [s.IDL_GTVT, s.IDL_GTVN]:
            self._combox[i].clear()
            self._combox[i].setEnabled(False)
            self._arrow_btn["prev.{}".format(i)].setEnabled(False)
            self._arrow_btn["next.{}".format(i)].setEnabled(False)
        ...
        for gtv in [s.GTVT, s.GTVN]:
            if self._idl_id[gtv] == s.BASELINE:
                reset_id = False
            elif (
    ...
    def __load_labels(self):
        ...
        self._gtvs_center = list(measurements.center_of_mass(labels[s.GTVS]))
        # float to int
    ...
    def _choose_idl_gtvt(
        ...
        self.__choose_idl(gtv=s.GTVT, reset_id=reset_id, refresh_imgs=refresh_imgs)

    def _choose_idl_gtvn(
        self, idx: int = None, reset_id: bool = True, refresh_imgs=True
    ):
        self.__choose_idl(gtv=s.GTVN, reset_id=reset_id, refresh_imgs=refresh_imgs)
    ...
    def __choose_idl(self, gtv: str, reset_id: bool = True, refresh_imgs: bool = True):
        ...
        if reset_id is True:
            ...
            if combox_item == s.BASELINE:
                self._idl_id[gtv] = s.BASELINE
                self._idl_round[gtv] = s.ROUND_00
            else:
        ...
        # triggered by patient combox update, and find cur patient in idl.gtvn dir
            ...
            if self._idl_id[gtv] == s.BASELINE:
                self._combox["idl.{}".format(gtv)].setCurrentText(s.BASELINE)
            else:
        ...
        if self._idl_id[gtv] == s.BASELINE:
            ...
            # clear gtvn clicks
            if gtv == s.GTVN:
                self._3d_imgs[s.GTVN_CLICKS] = None
        else:
            pred_path = os.path.join(
                g.TRAIN_RESULTS_DIR,
                self._baseline_id,
    ...
    def __choose_prev_baseline(self):
        idx = self._combox[s.BASELINE].currentIndex() - 1
        if idx < 0:
            return
        prev_baseline = self._combox[s.BASELINE].itemText(idx)
        self._combox[s.BASELINE].setCurrentText(prev_baseline)
        self._choose_baseline()

    def __choose_next_baseline(self):
        idx = self._combox[s.BASELINE].currentIndex() + 1
        if idx > self._combox[s.BASELINE].count() - 1:
            return
        next_baseline = self._combox[s.BASELINE].itemText(idx)
        self._combox[s.BASELINE].setCurrentText(next_baseline)
        self._choose_baseline()

    def __choose_prev_idl_gtvn(self):
        idx = self._combox[s.IDL_GTVN].currentIndex() - 1
        if idx < 0:
            return
        prev_idl_gtvn = self._combox[s.IDL_GTVN].itemText(idx)
        self._combox[s.IDL_GTVN].setCurrentText(prev_idl_gtvn)
        self._choose_idl_gtvn()

    def __choose_next_idl_gtvn(self):
        idx = self._combox[s.IDL_GTVN].currentIndex() + 1
        if idx > self._combox[s.IDL_GTVN].count() - 1:
            return
        next_idl_gtvn = self._combox[s.IDL_GTVN].itemText(idx)
        self._combox[s.IDL_GTVN].setCurrentText(next_idl_gtvn)
        self._choose_idl_gtvn()

    def __choose_prev_idl_gtvt(self):
        idx = self._combox[s.IDL_GTVT].currentIndex() - 1
        if idx < 0:
            return
        prev_idl_gtvt = self._combox[s.IDL_GTVT].itemText(idx)
        self._combox[s.IDL_GTVT].setCurrentText(prev_idl_gtvt)
        self._choose_idl_gtvt()

    def __choose_next_idl_gtvt(self):
        idx = self._combox[s.IDL_GTVT].currentIndex() + 1
        if idx > self._combox[s.IDL_GTVT].count() - 1:
            return
        next_idl_gtvt = self._combox[s.IDL_GTVT].itemText(idx)
        self._combox[s.IDL_GTVT].setCurrentText(next_idl_gtvt)
        self._choose_idl_gtvt()

    def __choose_prev_patient(self):
        idx = self._combox[s.PATIENT].currentIndex() - 1
        if idx < 0:
            return
        prev_patient = self._combox[s.PATIENT].itemText(idx)
        self._combox[s.PATIENT].setCurrentText(prev_patient)
        self._choose_patient()

    def __choose_next_patient(self):
        idx = self._combox[s.PATIENT].currentIndex() + 1
        if idx > self._combox[s.PATIENT].count() - 1:
            return
        next_patient = self._combox[s.PATIENT].itemText(idx)
        self._combox[s.PATIENT].setCurrentText(next_patient)
        self._choose_patient()
    ...
    def _refresh_rgb_imgs(self):
        ...
        # set contour color
        color = Dict()
        color[s.GTVT_LABEL] = self._color[s.GTVT_LABEL]
        color[s.GTVN_LABEL] = self._color[s.GTVN_LABEL]

        if is_annotated:
            color[s.GTVT_PRED] = self._color[s.GTVT_ANNOTATION]
        else:
            color[s.GTVT_PRED] = self._color[s.GTVT_PRED]

        color[s.GTVN_PRED] = self._color[s.GTVN_PRED]
        color[s.GTVN_CLICKS] = self._color[s.GTVN_CLICKS]
        ...
        for i in [s.CT, s.PT, s.MR1, s.MR2]:
            if self._img_plane == s.SAGITTAL:
                ...
                rgb_img = self._3d_imgs[i][:, self._cur_slice, :]
            elif self._img_plane == s.TRANSVERSE:
                rgb_img = self._3d_imgs[i][self._cur_slice, :, :]
            ...
            for direction in [s.HORIZONTAL, s.VERTICAL]:
                for gtvt_selected_slice_2d in self.__gtvt_selected_slices_2d[direction]:
                    ...
                    # 1mm images are reversed in sagittal plane
                        ...
                        self._img_plane != s.SAGITTAL
                        and direction == s.VERTICAL
                        and (
                            self._dataset_ver == s.AU_1MM or self._dataset_ver == s.MDA
                        )
                    ):
                        slice_pos = (
                            self.__total_slices_count_2d[direction]
                    ...
                    cur_slice_mask = cv2.rectangle(
                        ...
                        pt1=(x1, y1),
                        pt2=(x2, y2),
                        color=self._color[s.GTVT_ANNOTATION],
                        thickness=-1,
                    )
    ...
    def _add_label_text_on_rgb_img(self, rgb_img):
        ...
        self._cv_put_text(
            ...
            text=cv_text,
            pos=(pos_x, pos_y),
            color=self._color[s.SCORE_TEXT],
        )
        ...
        self._cv_put_text(
            ...
            text=cv_text,
            pos=(pos_x, pos_y),
            color=self._color[s.GTVT_LABEL],
        )
        ...
        self._cv_put_text(
            ...
            text=cv_text,
            pos=(pos_x, pos_y),
            color=self._color[s.GTVN_LABEL],
        )
    ...
    def __get_gtvt_selected_slices_on_2d(self, img_plane) -> List:
        ...
        if self._idl_round[s.GTVT] == s.ROUND_00:
            return []

        # load annotated slices
        idl_gtvt_dir = os.path.join(
            g.TRAIN_RESULTS_DIR, self._baseline_id, self._idl_id[s.GTVT]
        )
        ...
        for round_num in selected_slices_dict:
            ...
            if (round_num) == self._idl_round[s.GTVT]:
                break
    ...
    def __get_slices_count(self) -> int:
        if (self._3d_imgs[s.CT] is None) or (self._img_plane is None):
            return 0
        elif self._img_plane == s.SAGITTAL:
            return self._3d_imgs[s.CT].shape[2]
        elif self._img_plane == s.CORONAL:
            return self._3d_imgs[s.CT].shape[1]
        elif self._img_plane == s.TRANSVERSE:
            return self._3d_imgs[s.CT].shape[0]
        else:
            Debug.error_exit("self._img_plane value error")

    def _refresh_title(self):
        win_tital = "iDL.Tool "
        # if self._idl_round[s.GTVT] is not None:
    ...
    def __resize_img_qlabels(self):
        ...
        self._img_qlabel[s.CT].setGeometry(
            QRect(pos["x"][0], pos["y"][0], size["x"][0], size["y"][0])
        )
        self._img_qlabel[s.PT].setGeometry(
            QRect(pos["x"][1], pos["y"][0], size["x"][1], size["y"][0])
        )
        self._img_qlabel[s.MR1].setGeometry(
            QRect(pos["x"][0], pos["y"][1], size["x"][0], size["y"][1])
        )
        self._img_qlabel[s.MR2].setGeometry(
