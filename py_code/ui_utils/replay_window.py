...
from ui_utils.custom_combox import CustomComboBox
from ui_utils.img_frame import ImgFrame
from ui_utils.toggle_btn import ToggleButton
...
class ReplayWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setupUi(self)
        # load setting
        ui_settings = g.load_json(os.path.join(g.PROJ_DIR, "settings", "ui.json"))
        self._init_data(ui_settings)
        self._init_color(ui_settings)  # before init_widgets()
        self._init_widgets(ui_settings)  # after _init_data()
        # self.__init_zoomin()
    ...
    def _init_patients(self):
        ...
        for i in [
            ...
            dataset_split = g.load_json(g.DATASET_SPLIT_PATH[i])
            self._patients[i] = List(dataset_split[DatasetPart.TEST])

    def _init_data(
        self,
        ui_settings: Dict,  # this is for idl_window
    ):
        self._debug_mode = ui_settings["debug.mode"]
    ...
    def _init_color(self, ui_settings: Dict):
        ...
        for i in [
            ...
            self.color[i] = List(ui_settings["color.values"][i])
    ...
    def _init_widgets(self, ui_settings: Dict):
        ...
        self._init_widgets_annotation(ui_settings)
    ...
    def _load_multi_modal_imgs(self):
        ...
        for i in [Modal.CT, Modal.PT, Modal.MR1, Modal.MR2]:
            if self.dataset_ver in [DatasetVer.AU, DatasetVer.OBS_STUDY]:
                ...
                img_path[i] = "{}_{}.nii.gz".format(self._cur_patient, img_path[i])
                img_path[i] = os.path.join(g.DATASET_DIR[self.dataset_ver], img_path[i])
